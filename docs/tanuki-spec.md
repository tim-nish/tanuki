...
## Command, decision pass, and file layout — pointers (archived 2026-07-27)

The former Command, Decision-pass, and Files sections of this document
paraphrased contracts owned elsewhere and are archived verbatim in
`docs/archive/2026-07-27-tanuki-spec-paraphrase-layer.md` (story 5.14 /
issue #345; per the derived-surface rules in `docs/README.md`, this file is
canonical only for standing constraints, model tiers, configuration, and
the component/vocabulary contract). Authoritative surfaces:

- **Argument grammar, init onboarding, views, ingest** — `commands/tanuki.md`.
- **Command shape** (a bare word never drives; flags modify a drive) —
  `specs/spec-short-command-surface/SPEC.md` D6; the decision pass — D1
  there, plus `specs/spec-tanuki-solve/SPEC.md` D1/D3 for the consolidation
  taxonomy.
- **Read-only views** — `specs/spec-tanuki-view/SPEC.md`.
- **Repository and `~/.tanuki` state layout** — the two tables in
  `README.md` §"What's in this repo" (the canonical copy of what the archived
  tree restated).

**Canonical scenarios location** (unchanged, still governing here):
`~/.tanuki/scenarios/<target>.scenarios.json` is the **single supported
home** for a target's matrix — it sits next to the rest of that target's
state. No other path is read (an earlier ad-hoc `~/.claude/templates/tanuki/`
location is not supported and cost a config that was nearly lost). The
loaders (`tanuki-drive`, `tanuki-loop init`, `tanuki-scheduler`) fail closed
with this canonical path when the config is absent, so a misplaced file
surfaces immediately instead of a stale copy being read silently.
